...
public class MyReservationsController {
            ...
            ((MovieReservationWebSocketClient) wsClient).setMessageHandler(message -> {
                javafx.application.Platform.runLater(() -> {
                    if (message.startsWith("USER_BOOKINGS_LIST:")) {
                        ...
                        Type bookingListType = new TypeToken<List<classes.Booking>>(){}.getType();
                        List<classes.Booking> receivedBookings = gson.fromJson(jsonBookings, bookingListType);
    ...
    */
     ...
     * danych z serwera (np. jako zdeserializowana lista obiektów classes.Booking).
     * @param bookings Lista obiektów classes.Booking do wyświetlenia.
     */
    private void displayReservations(List<Booking> bookings) {
        reservationsListView.getItems().clear();
        if (bookings != null && !bookings.isEmpty()) {
            for (Booking booking : bookings) {
                // Sformatuj obiekt classes.Booking do Stringa, który będzie wyświetlany
